...
import { useNavigation, useRoute, getActionFromState } from '@react-navigation/native';
...
const Details = () => {
    ...
    useEffect(() => {
        async function getInfo() {
            const query = await api.get(`points/${routeParams.point_id}`);
            setData(query.data);            
        };
        ...
        <SafeAreaView style={{ flex: 1 }}>
            <View style={styles.container}>
                ...
                <Text style={styles.pointItems}>
                    {data.items.map(el => el.title).join(', ')}
                </Text>
